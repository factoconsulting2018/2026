...
$this->title = 'Gestión de Órdenes 2';
...
$this->registerCss('
    ...
    .zip-btn:hover, .zip-icon:hover {
        background: linear-gradient(135deg, #138496, #117a8b);
        transform: scale(1.05);
    }
    
    .zip-btn::before, .zip-icon::before {
        content: "";
        position: absolute;
        top: 0;
        left: -100%;
    ...
    .zip-btn:hover::before, .zip-icon:hover::before {
        left: 100%;
    }
...
<div class="rental-index">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?= Html::a('<span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle; margin-right: 4px;">add</span>Nueva Orden', ['create'], ['class' => 'btn btn-success']) ?>
    </div>
                            ...
                            <div class="<?= $rowClass ?>" data-estado="<?= $estado ?>">
                                ...
                                <div class="data-cell actions-cell">
                                    <div class="crud-actions">
                                        ...
                                        <button type="button" 
                                           class="action-btn zip-btn zip-btn-hide" 
                                           data-rental-id="<?= $model->id ?>"
                                           title="Descargar ZIP con PDF"
                                           onclick="downloadZip(<?= $model->id ?>)"
                                           style="display:none;">
                        ...
                        <div class="accordion-layout-container">
                            ...
                            <div class="accordion-actions-right">
                                ...
                                <button type="button" 
                                   ...
                                   data-rental-id="<?= $model->id ?>"
                                   title="Descargar PDF (mPDF)"
                                   onclick="downloadPdfAsync(<?= $model->id ?>)">
                                    <span class="material-symbols-outlined">file_present</span>
                                </button>
                                <button type="button" 
                                   class="action-icon zip-icon" 
                                   data-rental-id="<?= $model->id ?>"
                                   title="Descargar ZIP con PDF"
                                   onclick="downloadZip(<?= $model->id ?>)"
                                   style="display:none;">
...
<div class="modal fade" id="pdfGeneratingModal" data-bs-backdrop="static" data-bs-keyboard="false">
...
<script>
function downloadPdfAsync(rentalId) {
    // Mostrar modal de progreso
    var modal = new bootstrap.Modal(document.getElementById('pdfGeneratingModal'));
    modal.show();
    ...
    $.ajax({
        url: '/pdf/generate-mpdf-async?id=' + rentalId,
        method: 'POST',
        dataType: 'json',
        success: function(response) {
            if (response.success) {
                ...
                // Polling para verificar si está listo
                checkPdfReady(rentalId, response.downloadUrl, modal);
            } else {
                modal.hide();
                alert('Error al generar PDF: ' + response.message);
            }
        },
        error: function() {
            modal.hide();
...
function checkPdfReady(rentalId, downloadUrl, modal) {
    var attempts = 0;
    var maxAttempts = 30; // 30 segundos máximo
    
    var interval = setInterval(function() {
        attempts++;
        updateProgress(50 + (attempts * 1.5)); // Incrementar progreso
        
        $.ajax({
            url: '/pdf/check-pdf-status?id=' + rentalId,
            method: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.ready) {
            ...
            error: function() {
                clearInterval(interval);
                modal.hide();
                alert('Error al verificar el estado del PDF');
            }
        });
...
function updateProgress(percent) {
    ...
    bar.text(Math.round(percent) + '%');
}

// Función para descargar ZIP directamente sin mostrar preview
function downloadZip(rentalId) {
    // Crear un elemento <a> temporal para forzar descarga
    var link = document.createElement('a');
    link.href = '/pdf/download-zip?id=' + rentalId;
    link.download = ''; // Forzar descarga
    link.style.display = 'none';
    document.body.appendChild(link);
    link.click();
...
// Verificar si ZIP existe y mostrar botón
$(document).ready(function() {
    // Verificar cada botón ZIP
    $(".zip-btn-hide, .zip-icon").each(function() {
        var btn = $(this);
        var rentalId = btn.data("rental-id");
        
        if (!rentalId) {
            // Intentar obtener del botón padre o del atributo onclick
            var onclick = btn.attr('onclick');
            if (onclick) {
                var match = onclick.match(/downloadZip\((\d+)\)/);
                if (match) {
                    rentalId = match[1];
                }
            }
        ...
        if (rentalId) {
            ...
            $.ajax({
                url: "/pdf/check-zip-status?id=" + rentalId,
                type: "GET",
                dataType: 'json',
                success: function(response) {
                    if (response.ready) {
                        // Mostrar el botón si el ZIP existe
                        if (btn.hasClass("zip-btn-hide")) {
                            btn.removeClass("zip-btn-hide");
                            btn.css({
                                "display": "flex",
                                "visibility": "visible",
                                "opacity": "1"
                            });
                        } else if (btn.hasClass("zip-icon")) {
                            btn.css({
                                "display": "flex !important",
                                "visibility": "visible",
                                "opacity": "1"
